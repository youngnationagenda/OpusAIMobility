 ...
 *
 * Usage:
 *   node aws/deploy-frontend.js
 *
 * Optional — pass a Gemini key to update it in Secrets Manager too:
 *   node aws/deploy-frontend.js --gemini-key AIza...
 */
...
function aws(cmd, label) {
  return run(`aws ${cmd} --region ${REGION} --output json`, label || cmd.slice(0, 70));
}
...
// ── STEP 2 — Sync assets (long cache) ────────────────────────────────────────
console.log('\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
console.log(' STEP 2 — Syncing to S3');
console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');

// JS/CSS assets — long-lived cache (content-hashed filenames)
run(
  `aws s3 sync "${DIST_DIR}/assets" s3://${BUCKET}/assets/` +
  ` --delete` +
  ` --cache-control "public, max-age=31536000, immutable"` +
  ` --region ${REGION}`,
  'S3 sync: assets/ (1yr cache)'
);
...
// Any other root files (favicon, manifest, robots.txt etc.)
run(
...
if (geminiKey) {
  console.log(`║  🔑  Gemini key: updated in Secrets Manager`);
}
console.log('╚═══════════════════════════════════════════════════╝');
console.log('');
